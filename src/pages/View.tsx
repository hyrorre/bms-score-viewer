...
const View: Component = () => {
    ...
    switch (e) {
      ...
      case "1":
        setData({
          ...
          pattern:
            keys() >= 10
              ? validateKeyPattern(1, keys() / 2)[1].concat(
                  data().pattern.slice(keys() / 2)
                )
              : validateKeyPattern(1, keys())[1],
        });
        break;
      case "2":
        var result = validateKeyPattern(
          randomP1(),
          keys() >= 10 ? keys() / 2 : keys()
        );
        if (result[0]) {
          setData({
            ...
            pattern:
              keys() >= 10
                ? result[1].concat(data().pattern.slice(keys() / 2))
                : result[1],
          });
        } else {
  ...
  onMount(async () => {
    ...
    response = await fetch(
      `https://popn.wiki/_media/score/start/${queryParams.filename}`
    );
    if (!response.ok) {
        ...
        <div id={styles.header}>
          <div id={styles.header_info}>
            <span class={styles.title_texts}>
              <span id={styles.title}>{data().title}</span> /{" "}
              {data().artist}{" "}
            </span>
            <span id={styles.title_misc}>
              - bpm: {data().bpm} / Notes: {data().notes} / Total:{" "}
              {data().total} / Rank: {data().rank}
            </span>
          </div>
            ...
            <Sheet id="menu">
              <SheetTrigger class="pr-1">
                <p>
                  <FaSolidGear
                    size={"1.2em"}
                    class="hover:animate-[spin_3s_linear_infinite]"
                  />
                </p>
              </SheetTrigger>
              <SheetContent class="w-[300px] overflow-auto">
                ...
                <div>
                  ...
                  {keys() !== 9 ? (
                    <div class="space-y-3 pt-3">
                      <Label for="playside_button">
                        {keys() === 10 || keys() === 14 ? "Flip" : "Side"}
                      </Label>
                      <RadioGroup
                        id="playside_button"
                  ...
                  <div class="space-y-3 pt-3">
                    <Label for="option_button">
                      {keys() >= 10 ? "Option 1P" : "Option"}
                    </Label>
                    <RadioGroup
                      id="option_button"
                      value={
                        viewParams().o >= 2 ? "2" : viewParams().o.toString()
                      }
                      onChange={(e) => {
                  ...
                  <div class="space-y-3 pt-3">
                    ...
                    <RadioGroup
                      id="color_button"
                      defaultValue={
                        viewParams().c.charAt(0).toUpperCase() +
                        viewParams().c.slice(1)
                      }
                      onChange={(e) => {
                  ...
                  <div class="menu_box pt-3">
                    ...
                    <div class="text-sm text-muted-foreground">
                      <p>
                        ...
                        <a href="http://ribbit.xyz/bms/score/">
                          BMS Score Viewer
                        </a>
                        の非公認ミラーサイトです。
                      </p>
                ...
                <SheetFooter class="text-xs text-muted-foreground mt-3">
                  ...
                  <div class="text-center italic">
                    Based on{" "}
                    <a href="https://github.com/ladymade-star/bms-score-viewer">
                      ladymade-star's mirror
                    </a>
                  </div>
                </SheetFooter>
              </SheetContent>
