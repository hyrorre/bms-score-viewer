...
const View: Component = () => {
  ...
  const updatePattern = (e: string, keyChange: boolean = false) => {
    // pattern is null for keys < 10 if nonran for optimization purposes
    ...
    switch (e) {
      case '0':
        setData({
          ...
          pattern:
            keys() >= 10
              ? default_pattern.concat(data().pattern.slice(keys() / 2))
              : keyChange
                ? default_pattern
                : null!,
        })
      ...
      case '2':
        ...
        if (result[0]) {
          setData({
            ...data(),
            pattern: keys() >= 10 ? result[1].concat(data().pattern.slice(keys() / 2)) : result[1],
          })
        } else {
    ...
    switch (e) {
      case '0':
        setData({
          ...data(),
          pattern: data()
            .pattern.slice(0, keys() / 2)
            .concat(default_pattern),
        })
        break
      case '1':
        setData({
          ...
          pattern: data()
            .pattern.slice(0, keys() / 2)
            .concat(validateKeyPattern(1, keys() / 2)[1]),
        })
        break
      case '2':
  ...
  const load = () => {
    ...
    if (randP1) setRandomP1(randP1)
    if (randP2) setRandomP2(randP2)
    setData(data)
    setKeys(data.keys)
  ...
  onMount(async () => {
    ...
    response = await fetch(`https://popn.wiki/_media/score/start/${queryParams.filename}`)
    if (!response.ok) {
      ...
      <div class={styles.body}>
        <div id={styles.header}>
          <div id={styles.header_info}>
            <span class={styles.title_texts}>
              <span id={styles.title}>{data().title}</span> / {data().artist}{' '}
            </span>
          ...
          <div id={styles.header_bars}>
            <span />
            <Sheet id="menu">
              <SheetTrigger class="pr-1">
                <p>
                  <FaSolidGear size={'1.2em'} class="hover:animate-[spin_3s_linear_infinite]" />
                </p>
              </SheetTrigger>
              <SheetContent class="w-[300px] overflow-auto">
                  ...
                  <SheetDescription>
                    <div id="menu_header">
                      <Grid cols={5} colsMd={5} class="w-full gap-1">
                        ...
                        <Col>
                          <A id="link_to_home" href="/" class={buttonVariants({ variant: 'secondary' })}>
                            <FaSolidHouseChimney class={styles['fa-home']} />
                          </A>
                        </Col>
                        <Col>
                          <A
                          ...
                          >
                            <FaBrandsTwitter class={styles['fa-twitter']} />
                          </A>
                        </Col>
                        <Col>
                          <A
                            ...
                            class={buttonVariants({ variant: 'secondary' })}
                          >
                            <FaSolidCamera class={styles['fa-camera']} />
                          </A>
                        </Col>
                      </Grid>
                    </div>
                ...
                <div>
                  ...
                  {keys() !== 9 ? (
                    <div class="space-y-3 pt-3">
                      ...
                      <RadioGroup
                        id="playside_button"
                        defaultValue={viewParams().p.toString()}
                        onChange={(e) => {
                  ...
                  <div class="space-y-3 pt-3">
                    ...
                    <RadioGroup
                      id="option_button"
                      value={viewParams().o >= 2 ? '2' : viewParams().o.toString()}
                      onChange={(e) => {
                  ...
                  {keys() >= 10 ? (
                    <div class="space-y-3 pt-3">
                      ...
                      <RadioGroup
                        id="option_button_2p"
                        value={viewParams().o2 >= 2 ? '2' : viewParams().o2.toString()}
                        onChange={(e) => {
                      ...
                      <div id="random_pattern_2p">
                        ...
                        <button
                          type="submit"
                          id="random_pattern_auto_button_2p"
                          class="inline px-2"
                          onClick={() => {
                            setRandomP2(randomizeKeyPatternStr(keypatInit[keys()], keys() >= 10 ? keys() / 2 : keys()))
                            setViewParams({ ...viewParams(), o2: 2 })
                  ...
                  <div class="space-y-3 pt-3">
                    ...
                    <RadioGroup
                      id="color_button"
                      defaultValue={viewParams().c.charAt(0).toUpperCase() + viewParams().c.slice(1)}
                      onChange={(e) => {
                        setViewParams({ ...viewParams(), c: e.toLowerCase() })
                        setQueryParams({
                          c: e === 'Default' ? '' : e.toLowerCase(),
                        })
                      }}
                    >
                      <Grid cols={2} class="w-full gap-2">
                  ...
                  <div class="menu_box pt-3">
                    ...
                    <div class="text-sm text-muted-foreground">
                      <p>
                        このアプリケーションは
                        <a href="https://github.com/rib2bit">@rib2bit</a>様の
                        <a href="http://ribbit.xyz/bms/score/">BMS Score Viewer</a>
                        の非公認ミラーサイトです。
                      </p>
                      <p>
                        静的サイトとしての公開に伴うソースコードの改変部分について一切の著作者人格権を行使しません。
                      </p>
                      <br />
                    </div>
                  </div>
                ...
                <SheetFooter class="text-xs text-muted-foreground mt-3">
                  <div class="text-center mt-1">
                    <a href="https://github.com/SayakaIsBaka/bms-score-viewer">
                      <img class="inline" src={GitHubLogo} style={{ height: '1rem', width: '1rem' }} />{' '}
                      SayakaIsBaka/bms-score-viewer
                    </a>
                  </div>
                  <div class="text-center italic">
                    Based on <a href="https://github.com/ladymade-star/bms-score-viewer">ladymade-star's mirror</a>
                  </div>
                </SheetFooter>
              </SheetContent>
            </Sheet>
